...
function listenBuyButtonClick(data, index) {
    document.querySelector(".btn-buy").addEventListener('click', () => {
        ...
        document.querySelector(".product-amount").innerHTML = `Amount bought:<strong>${data[index].rating.count}</strong>`
    })
};

function viewSingleProductPage(data, index) {
    document.getElementById("shop-container").classList.remove("shop-container");
    document.getElementById("shop-container").classList.add("single-product-view");

    document.getElementById("shop-container").innerHTML = `
        <div class="product-container">
            <h2 class="product-title">${data[index].title}</h2>
            <img class="product-img" src="${data[index].image}" alt="product image">
            <div class="center-container">
                <button class="btn-buy">Buy</button>
            </div>
            <p>Codigo: <strong>${data[index].id}</strong></p>
            <p class="product-desc">${data[index].description}</p>
            <p class="p-rate">Rating:<strong>${data[index].rating.rate}</strong></p>
            <p class="product-amount p-rate">Amount bought:<strong>${data[index].rating.count}</strong></p>
            <p class="p-rate">Price<strong class="product-price">$${data[index].price}</strong></p>
...
async function fetchItems() {
    document.getElementById("shop-container").classList.remove("single-product-view");
